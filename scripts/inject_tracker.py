...
# Build the Astro page using string concatenation (avoid f-string interpolation issues)
header = """---
...
<script>
  ...
  var canvasEl = document.getElementById('qi');
  if (!canvasEl) {
  ...
  var qiCanvas = document.getElementById('qi');
  if (qiCanvas) {
    var ctx = qiCanvas.getContext('2d');
    var pts = [];
    function resize() {
    ...
    function spawn(x, y) {
      for (var i = 0; i < 5; i++)
        pts.push({x:x, y:y, tx:x+(Math.random()-.5)*60, ty:y+(Math.random()-.5)*60, life:35+Math.random()*20, r:1+Math.random()*2});
      if (pts.length > 120) pts = pts.slice(-120);
    }
    window.addEventListener('mousemove', function(e) { spawn(e.clientX, e.clientY); });
    function tick() {
...
Path("src/pages/index.astro").write_text(header, encoding="utf-8")
print("Astro page updated successfully with", len(body_content), "chars of tracker body")